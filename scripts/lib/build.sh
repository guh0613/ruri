# Shared implementation for the build entry points; source from the repository root.
select_xcode() {
  local xcode
...
compile_app_icon() {
  local app="$1" icon="$2" icon_name="$3" generated="$4"
  xcrun actool "$icon" --compile "$app/Contents/Resources" \
    ...
    --app-icon "$icon_name" --output-format human-readable-text \
    --output-partial-info-plist "$generated"
  python3 - "$app/Contents/Info.plist" "$generated" <<'PY'
import plistlib, sys
...
# Run in a subshell so staging cleanup and variables stay local to the host build.
build_game_host() (
...
PY
  ...
  compile_app_icon "$app" Resources/GameHostIcon.icon GameHostIcon "$stage_dir/icon-info.plist"
  sign_options=()
