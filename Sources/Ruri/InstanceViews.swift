...
struct LogsView: View {
    ...
    var lines: [String] { model.logs.filter { filter.isEmpty || $0.localizedCaseInsensitiveContains(filter) } }
    var body: some View {
        VStack(spacing: 14) {
                ...
                ScrollView([.vertical, .horizontal]) {
                    LazyVStack(alignment: .leading, spacing: 3) {
                        ForEach(Array(lines.enumerated()), id: \.offset) { index, line in
                            Text(line).font(.system(size: 11, design: .monospaced)).foregroundStyle(line.contains("ERROR") || line.contains("Exception") ? .orange : .primary).textSelection(.enabled).id(index)
                        }
                        Color.clear.frame(height: 1).id("end")
                    }.padding(14)
                }.background(Color(nsColor: .textBackgroundColor), in: RoundedRectangle(cornerRadius: 10))
                    .onChange(of: model.logs.count) { if follow { proxy.scrollTo("end", anchor: .bottom) } }
            }
            HStack { Text("显示最近 5,000 行；完整日志保存在实例的 launcher.log。").font(.caption).foregroundStyle(.secondary); Spacer(); if model.runningID != nil { Button("结束游戏", role: .destructive) { model.stopGame() } } }
        }.padding(24).frame(width: 880, height: 610)
    }
    private func export() {
        let panel = NSSavePanel(); panel.nameFieldStringValue = "ruri-game.log"; panel.allowedContentTypes = [.plainText]
        if panel.runModal() == .OK, let url = panel.url { do { try model.logs.joined(separator: "\n").write(to: url, atomically: true, encoding: .utf8) } catch { model.error = error.localizedDescription } }
    }
}
