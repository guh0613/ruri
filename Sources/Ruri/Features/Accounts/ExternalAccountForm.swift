...
struct ExternalAccountForm: View {
            ...
            if let server {
                ...
                if let pending {
                    ...
                    Button(Messages.AppExternalAccountForm.addRole.localized) { selectProfile(pending, server: server) }
                        .buttonStyle(.borderedProminent).keyboardShortcut(.defaultAction).disabled(task != nil || profileID.isEmpty)
                } else {
                    LabeledContent(Messages.AppExternalAccountForm.accountName.localized) { TextField(Messages.AppExternalAccountForm.accountName.localized, text: $username).textFieldStyle(.roundedBorder).disabled(existing != nil || task != nil) }
                    ...
                    Button(existing == nil ? Messages.AppExternalAccountForm.login.localized : Messages.AppExternalAccountForm.relogin.localized) { login(server) }
                        .buttonStyle(.borderedProminent).keyboardShortcut(.defaultAction).disabled(task != nil || username.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty || password.isEmpty)
                }
            } else {
                ...
                Button(Messages.AccountCenter.useLittleSkin.localized) { address = "https://littleskin.cn/api/yggdrasil/"; discover() }.disabled(task != nil)
                ...
                Button(Messages.AppExternalAccountForm.identifyServer.localized) { discover() }.buttonStyle(.borderedProminent).keyboardShortcut(.defaultAction).disabled(address.isEmpty || task != nil)
            }
    ...
    private func login(_ server: ExternalAuthServer) {
        ...
        run {
            ...
            if result.selectedProfile != nil { try await finish(result, server: server, username: loginName) }
    ...
    private func selectProfile(_ pending: ExternalAuthSession, server: ExternalAuthServer) {
        ...
        run {
            ...
            try await finish(result, server: server, username: username.trimmingCharacters(in: .whitespacesAndNewlines))
        }
    }
    private func finish(_ result: ExternalAuthSession, server: ExternalAuthServer, username: String) async throws {
        try Task.checkCancellation()
