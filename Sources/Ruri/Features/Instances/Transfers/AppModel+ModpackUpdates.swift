...
extension AppModel {
    func prepareModpackUpdate(_ prepared: PreparedInstanceImport, instance: GameInstance, keepJVMArguments: Bool, curseFiles: [PlannedCurseFile], manualFiles: [Int: URL],
        ...
        perform(Messages.AppAppModelModpackUpdates.prepareModpackUpdateText1(String(describing: instance.name))) { [self] id in
    ...
    func applyModpackUpdate(_ plan: PreparedModpackUpdate, keeping: Set<String>, completion: @MainActor @Sendable @escaping () -> Void) {
        save()
        perform(Messages.AppAppModelModpackUpdates.applyModpackUpdateText1(String(describing: plan.instance.name))) { [self] id in
            ...
            acceptState(saved); notice = Messages.AppAppModelModpackUpdates.savedText1(String(describing: plan.instance.name), String(describing: plan.incoming.version)).localized; completion()
        }
    }
    func rollbackModpack(_ instance: GameInstance, completion: @MainActor @Sendable @escaping () -> Void) {
        save()
        perform(Messages.AppAppModelModpackUpdates.rollbackModpackText1(String(describing: instance.name))) { [self] _ in
            let result = try await ModpackUpdater(paths: paths).rollback(instance)
            acceptState(result.state)
            notice = result.preservedFiles == 0 ? Messages.AppAppModelModpackUpdates.resultText1.localized : Messages.AppAppModelModpackUpdates.resultText2(Int64(result.preservedFiles)).localized
            completion()
        }
    }
    func recoverModpackUpdate(_ instance: GameInstance, completion: @MainActor @Sendable @escaping () -> Void) {
        perform(Messages.AppAppModelModpackUpdates.recoverModpackUpdateText1(String(describing: instance.name))) { [self] _ in
            let paths = paths
            ...
            acceptState(saved); notice = Messages.AppAppModelModpackUpdates.savedText2.localized; completion()
        }
    }
}
