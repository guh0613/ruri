...
extension AppModel {
    func installCurseForge(_ plan: CurseForgeContentPlan, manualFiles: [Int: URL]) {
        ...
        perform(Messages.AppAppModelCurseForge.installCurseForgeText1(String(describing: plan.title)), instanceID: plan.instance.id) { [self] id in
            ...
            notice = Messages.AppAppModelCurseForge.installCurseForgeText2(String(describing: plan.title)).localized
        }
    }
    func readCurseForgePack(_ project: CurseForgeProject, file: CurseForgeFile, manual: URL?) {
        perform(Messages.AppAppModelCurseForge.readCurseForgePackText1(String(describing: project.name))) { [self] id in
            let archive: URL
            if let manual { archive = manual }
            else {
                guard project.allowModDistribution != false, let url = file.downloadURL else { throw RuriError.message(Messages.AppAppModelCurseForge.urlText1) }
            ...
            guard DownloadManager.valid(archive, item: try file.downloadItem(to: archive, permittedURL: nil)) else { throw RuriError.message(Messages.AppAppModelCurseForge.urlText2) }
