...
struct ContentBatchUpdateView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 18) {
            SectionHeading(title: Messages.AppContentBatchUpdateView.updateItems(Int64(plan.selectedIDs.count)).localized, subtitle: plan.instance.name)
            Text(Messages.AppContentBatchUpdateView.fileSummary(Int64(plan.records.count), String(describing: LocalizedFormat.bytes(plan.downloadSize))).localized).font(.callout).foregroundStyle(.secondary)
            ScrollView {
                    ...
                    ForEach(plan.records) { record in
                        VStack(alignment: .leading, spacing: 5) {
                            HStack {
                                ...
                                Spacer()
                                if !plan.selectedIDs.contains(record.id) { TagPill(text: Messages.AppContentBatchUpdateView.requiredDependencies.localized) }
                            }
                            if let previous = plan.baseline.first(where: { $0.id == record.id }) {
                                Text(previous.versionName + " → " + record.versionName).font(.callout)
                                if !previous.enabled { Text(Messages.AppContentBatchUpdateView.keepDisabled.localized).font(.caption).foregroundStyle(.secondary) }
                            } else { Text(Messages.AppContentBatchUpdateView.newVersion(record.versionName).localized).font(.callout) }
            ...
            Text(Messages.AppContentBatchUpdateView.replacementDetails.localized).font(.caption).foregroundStyle(.secondary)
            HStack {
