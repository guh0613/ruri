...
extension AppModel {
    func refreshSessions(useCache: Bool = false) async {
        ...
        for record in result.0 where merged[record.id].map({ record.isAtLeastAsRecent(as: $0) }) ?? true { merged[record.id] = record }
        ...
        if sessions != updated { sessions = updated; historyRevision = UUID() }
    ...
    func publishSession(_ record: GameSession) {
        if let index = sessions.firstIndex(where: { $0.id == record.id }) {
            guard sessions[index] != record, record.isAtLeastAsRecent(as: sessions[index]) else { return }
            sessions[index] = record
        } else { sessions.insert(record, at: 0) }
        historyRevision = UUID()
        if inspectedSession?.id == record.id { inspectedSession = record }
    }
}
