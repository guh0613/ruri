...
@MainActor @Observable final class AppModel {
    ...
    var selectedDirectoryName: String { state.gameDirectories?.first(where: { $0.id == selectedDirectoryID })?.name ?? Messages.AppAppModel.selectedDirectoryNameText1.localized }
    ...
    init() {
        ...
        catch { state = PersistentState(); self.error = Messages.AppAppModel.rootText1(String(describing: error.localizedDescription)).localized; readOnly = true }
    }
    func save() {
        ...
        do { state = try StateStore.save(state, to: paths, basedOn: persistedState); persistedState = state }
            ...
            readOnly = true
            self.error = Messages.AppAppModel.saveText1(String(describing: error.localizedDescription)).localized
        }
    }
    func boot() async {
    ...
    private func initializeApplication() async {
        if !readOnly {
            ...
            } catch { self.error = Messages.AppAppModel.basePathsText1(String(describing: error.localizedDescription)).localized }
        }
        if !readOnly {
    ...
    func perform(_ title: LocalizedMessage, presentErrors: Bool = true, instanceID: UUID? = nil, work: @escaping @MainActor @Sendable (UUID) async throws -> Void) {
        ...
        operation = Task {
            ...
            do {
                ...
                if let i = activities.firstIndex(where: { $0.id == activity.id }) { activities[i].status = .completed; activities[i].progress = InstallProgress(Messages.AppAppModel.iText1, completed: 1, total: 1) }
            } catch {
                if let i = activities.firstIndex(where: { $0.id == activity.id }) {
                    activities[i].status = Task.isCancelled ? .cancelled : .failed
                    activities[i].error = error is RunDirectoryCopyFailure || error is InstanceMoveFailure || error is RepositoryImportFailure ? error.localizedDescription : Task.isCancelled ? Messages.AppAppModel.iText2.localized : error.localizedDescription
                }
    ...
    func synchronizeExternalState() async {
        ...
        do {
            ...
            if baselineRevision != nil && remote.revision == nil { throw RuriError.message(Messages.AppAppModel.remoteText1) }
            if state == persistedState { state = remote; persistedState = remote }
            else { save() }
        } catch { readOnly = true; self.error = Messages.AppAppModel.remoteText2(String(describing: error.localizedDescription)).localized }
    }
