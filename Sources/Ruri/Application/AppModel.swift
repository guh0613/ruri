        ...
        operation = Task {
            ...
            do {
                ...
                if let i = activities.firstIndex(where: { $0.id == activity.id }) {
                    ...
                    activities[i].error = error is RunDirectoryCopyFailure || error is InstanceMoveFailure || error is RepositoryImportFailure ? error.localizedDescription : Task.isCancelled ? "任务已取消。重试时会复用可用缓存，并尝试继续未完成的下载。" : error.localizedDescription
                }
