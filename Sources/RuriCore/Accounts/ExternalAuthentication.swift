...
public struct ExternalAuthServer: Codable, Equatable, Sendable {
    ...
    public static func address(_ input: String) throws -> URL {
        ...
        guard var components = URLComponents(string: text), components.scheme?.lowercased() == "https",
            ...
            throw RuriError.message("请输入不含账号、查询参数或片段的 HTTPS 认证服务器地址。")
        }
        components.scheme = "https"; components.host = host.lowercased()
        if components.port == 443 { components.port = nil }
        if components.path.isEmpty { components.path = "/" }
        guard let url = components.url else { throw RuriError.message("认证服务器地址无效。") }
        return url
    }
}
...
public struct ExternalAuthProfile: Codable, Identifiable, Equatable, Sendable {
    ...
    func validate() throws {
        guard normalizedID.range(of: "^[0-9a-f]{32}$", options: .regularExpression) != nil,
            ...
            throw RuriError.message("认证服务器返回了无效的角色信息。")
        }
    }
}
...
public struct ExternalAuthSession: Decodable, Sendable {
    ...
    public func account(server: ExternalAuthServer, username: String, replacing existing: Account? = nil) throws -> Account {
        guard let profile = selectedProfile else { throw RuriError.message("请先选择一个游戏角色。") }
        ...
        if let existing {
            guard existing.kind == .external, existing.externalLogin?.server.url == server.url,
                ...
                throw RuriError.message("原来的角色已不可用，请重新添加账号并选择角色。")
            }
        }
...
public struct ExternalAuthentication: Sendable {
    ...
    public func metadata(for server: ExternalAuthServer) async throws -> ExternalAuthMetadata {
        ...
        guard response.url == url else { throw RuriError.message("认证服务器地址已变更，请重新添加账号。") }
    ...
    public func login(server: ExternalAuthServer, username: String, password: String) async throws -> ExternalAuthSession {
        guard !username.isEmpty, !password.isEmpty else { throw RuriError.message("请填写认证站账号和密码。") }
        ...
        guard result.selectedProfile != nil || result.availableProfiles?.isEmpty == false else {
            throw RuriError.message("这个账号还没有游戏角色，请先到认证站创建角色。")
        }
        return result
    }
    ...
    public func select(_ profile: ExternalAuthProfile, from pending: ExternalAuthSession, server: ExternalAuthServer) async throws -> ExternalAuthSession {
        guard pending.selectedProfile == nil, pending.availableProfiles?.contains(profile) == true else { throw RuriError.message("请重新登录并选择可用角色。") }
        ...
        guard result.selectedProfile?.normalizedID == profile.normalizedID else { throw RuriError.message("认证服务器未返回所选角色，请重新登录。") }
        return result
    }
    ...
    public func refresh(account: Account, credentials: ExternalAccountCredentials, force: Bool = false) async throws -> (Account, ExternalAccountCredentials) {
        guard let login = account.externalLogin, account.kind == .external else { throw RuriError.message("账号缺少认证服务器信息，请重新添加。") }
        ...
        if !force {
            ...
            guard [401, 403].contains(response.statusCode) else { try requireSuccess(response.statusCode); throw RuriError.message("认证服务器返回了无效的令牌校验结果。") }
        }
    ...
    private func exchange(_ server: ExternalAuthServer, action: String, body: [String: Any], clientToken: String) async throws -> ExternalAuthSession {
        ...
        guard value.clientToken == clientToken, !value.accessToken.isEmpty, !value.accessToken.contains("\0") else { throw RuriError.message("认证服务器返回了无效的登录凭据。") }
    ...
    private func metadata(_ data: Data, url: URL) throws -> ExternalAuthMetadata {
        ...
        guard let value = try? JSONDecoder().decode(Metadata.self, from: data) else { throw RuriError.message("此地址没有提供外置认证信息，请检查认证站给出的 API 地址。") }
        guard value.signaturePublickey.contains("-----BEGIN PUBLIC KEY-----") else { throw RuriError.message("此地址没有提供有效的外置认证元数据。") }
        var root = url
        ...
        return .init(server: .init(url: root, name: name.isEmpty ? (root.host ?? "认证服务器") : String(name.prefix(120))), data: data)
    }
    private func requireSuccess(_ status: Int) throws {
        guard (200..<300).contains(status) else {
            if [401, 403].contains(status) { throw RuriError.message("认证失败：账号密码不正确，或登录已失效，请重新登录。") }
            throw RuriError.message("认证服务器返回 HTTP \(status)，请稍后重试。")
        }
    }
    private func request(_ url: URL, body: [String: Any]? = nil) async throws -> (Data, HTTPURLResponse) {
        ...
        guard let response = response as? HTTPURLResponse, data.count <= 65_536 else { throw RuriError.message("认证服务器返回的数据无效或过大。") }
