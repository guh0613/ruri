...
public struct AccountAppearanceClient: Sendable {
    ...
    public func load() async throws -> AccountAppearance {
        switch account.kind {
        case .offline: throw RuriError.message(Messages.CoreAccountAppearance.loadText1)
        case .microsoft:
            struct Profile: Decodable {
                struct Texture: Decodable {
                    let id: String; let state: String; let url: URL; let variant: String?; let alias: String?
                    var texture: AccountTexture { .init(id: id, name: alias ?? Messages.CoreAccountAppearance.textureText1.localized, url: url, active: state == "ACTIVE", model: variant?.lowercased() == "slim" ? .slim : .classic) }
                }
        ...
        case .external:
            ...
            if let encoded = properties.first(where: { $0.name == "textures" })?.value {
                guard let data = Data(base64Encoded: encoded) else { throw RuriError.message(Messages.CoreAccountAppearance.dataText1) }
                let payload = try JSONDecoder().decode(Textures.self, from: data)
                if let id = payload.profileId { try validateID(id) }
                if let texture = payload.textures["SKIN"] { skin = .init(id: "skin", name: Messages.CoreAccountAppearance.textureText2.localized, url: texture.url, active: true, model: texture.metadata?["model"] == "slim" ? .slim : .classic) }
                if let texture = payload.textures["CAPE"] { capes = [.init(id: "cape", name: Messages.CoreAccountAppearance.textureText3.localized, url: texture.url, active: true, model: .classic)] }
            }
    ...
    public func upload(_ image: PlayerTextureImage, kind: PlayerTextureKind, model: PlayerSkinModel = .classic, expecting appearance: AccountAppearance) async throws {
        ...
        guard appearance.uploadable.contains(kind) else { throw RuriError.message(Messages.CoreAccountAppearance.uploadText1(String(describing: kind.title))) }
    ...
    public func reset(_ kind: PlayerTextureKind, expecting appearance: AccountAppearance) async throws {
        try validate(appearance)
        if account.kind == .external, !appearance.uploadable.contains(kind) { throw RuriError.message(Messages.CoreAccountAppearance.resetText1(String(describing: kind.title))) }
    ...
    public func selectCape(_ id: String, expecting appearance: AccountAppearance) async throws {
        try validate(appearance)
        guard account.kind == .microsoft, appearance.capes.contains(where: { $0.id == id }) else { throw RuriError.message(Messages.CoreAccountAppearance.selectCapeText1) }
    ...
    public func image(for texture: AccountTexture) async throws -> PlayerTextureImage {
        var url = texture.url
        guard ["https", "http"].contains(url.scheme), url.host != nil, url.user == nil, url.password == nil else { throw RuriError.message(Messages.CoreAccountAppearance.urlText1) }
        ...
        guard let response = response as? HTTPURLResponse, (200..<300).contains(response.statusCode) else { throw RuriError.message(Messages.CoreAccountAppearance.responseText1) }
    ...
    private func validate(_ appearance: AccountAppearance) throws {
        guard account.kind != .offline, appearance.account.id == account.id, appearance.account.uuid == account.uuid,
              appearance.account.kind == account.kind, appearance.account.externalLogin == account.externalLogin else { throw RuriError.message(Messages.CoreAccountAppearance.validateText1) }
    ...
    private func validateID(_ value: String) throws {
        ...
        guard normalized == account.uuid, normalized.range(of: "^[0-9a-f]{32}$", options: .regularExpression) != nil else { throw RuriError.message(Messages.CoreAccountAppearance.normalizedText1) }
    }
    private func external(_ path: String) throws -> URL {
        try validateID(account.uuid)
        guard let server = account.externalLogin?.server else { throw RuriError.message(Messages.CoreAccountAppearance.serverText1) }
    ...
    private func api(_ url: URL, method: String = "GET", body: Data? = nil, contentType: String? = nil, authenticated: Bool = true) async throws -> Data {
        ...
        let (data, response) = try await session.data(for: request, delegate: redirects)
        guard let status = (response as? HTTPURLResponse)?.statusCode else { throw RuriError.message(Messages.CoreAccountAppearance.statusText1) }
        guard (200..<300).contains(status) else {
            switch status {
            case 401: throw RuriError.message(Messages.CoreAccountAppearance.statusText2)
            case 403: throw RuriError.message(Messages.CoreAccountAppearance.statusText3)
            case 429: throw RuriError.message(Messages.CoreAccountAppearance.statusText4)
            default: throw RuriError.message(Messages.CoreAccountAppearance.statusText5(String(describing: status)))
            }
        }
        guard data.count <= 262_144 else { throw RuriError.message(Messages.CoreAccountAppearance.statusText6) }
        return data
    }
}
