...
public enum GamePlaytimeStore {
    public static func load(paths: LauncherPaths, instanceID: UUID) throws -> GamePlaytime? {
        ...
        guard attributes.isRegularFile == true, (attributes.fileSize ?? .max) <= 8_388_608 else { throw RuriError.message("游玩时长记录无效。") }
        ...
        guard record.base.isFinite, record.base >= 0, record.sessions.count <= 100_000,
              record.sessions.allSatisfy({ UUID(uuidString: $0.key) != nil && $0.value.isFinite && $0.value >= 0 }) else { throw RuriError.message("游玩时长记录无效。") }
        return record
    }
