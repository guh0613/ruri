...
struct OptiFineInstaller: Sendable {
    ...
    func install(instance: GameInstance, base: VersionManifest, progress: @Sendable @escaping (InstallProgress) async -> Void) async throws -> VersionManifest {
        guard base.mainClass == "net.minecraft.client.main.Main" else { throw RuriError.message(Messages.CoreOptiFineInstaller.installText1) }
        guard let requested = instance.loaderVersion,
            ...
            throw RuriError.message(Messages.CoreOptiFineInstaller.releaseText1)
        }
        let file = try LauncherPaths.safePath("installers/optifine-\(instance.gameVersion)-\(release.version).jar", within: paths.cache)
        await progress(InstallProgress(Messages.CoreOptiFineInstaller.fileText1))
    ...
    func repair(instance: GameInstance, manifest: VersionManifest, progress: @Sendable @escaping (InstallProgress) async -> Void) async throws {
        ...
        guard let installer = manifest.generatedLibraries?.first(where: { $0.path?.hasSuffix("-installer.jar") == true && $0.path?.hasPrefix("optifine/OptiFine/") == true }),
              let version = instance.loaderVersion else { throw RuriError.message(Messages.CoreOptiFineInstaller.versionText1) }
    ...
    private func generate(instance: GameInstance, base: VersionManifest, installer file: URL, version: String, sourceURL: URL?, expected: [Artifact]? = nil,
        ...
        guard game == instance.gameVersion, metadata.version == version else { throw RuriError.message(Messages.CoreOptiFineInstaller.gameText1) }
        ...
        if archive["optifine/Patcher.class"] != nil {
            guard let client = base.downloads?["client"] else { throw RuriError.message(Messages.CoreOptiFineInstaller.clientText1) }
            ...
            if let existing = try? JavaDiscovery.select(from: runtimes, major: major, architecture: architecture) { java = existing }
                ...
                guard let runtime = try await service.available().first(where: { $0.major == major && $0.architecture == architecture }) else { throw RuriError.message(Messages.CoreOptiFineInstaller.runtimeText1(String(describing: major))) }
                java = try await service.install(runtime, downloader: downloader, progress: progress); runtimes.append(java)
            }
            await progress(InstallProgress(Messages.CoreOptiFineInstaller.runtimeText2))
            ...
            guard status == 0 else { throw RuriError.message(Messages.CoreOptiFineInstaller.statusText1(String(describing: status), String(describing: log.path))) }
        ...
        if archive["launchwrapper-of.txt"] != nil {
            ...
            guard wrapperVersion.range(of: "^[A-Za-z0-9._-]+$", options: .regularExpression) != nil else { throw RuriError.message(Messages.CoreOptiFineInstaller.wrapperVersionText1) }
        ...
        let prepared = try outputs.map { name, path, source -> (Library, URL) in
            ...
            if let expected {
                guard let original = expected.first(where: { $0.path == path }), original.sha1 == artifact.sha1 else { throw RuriError.message(Messages.CoreOptiFineInstaller.originalText1) }
            }
    ...
    static func metadata(_ archive: Archive) throws -> (game: String, version: String) {
        guard let name = ["Config.class", "net/optifine/Config.class", "notch/net/optifine/Config.class"].first(where: { archive[$0] != nil }) else { throw RuriError.message(Messages.CoreOptiFineInstaller.nameText1) }
        let values = try OptiFineClassMetadata.values(read(name, in: archive))
        guard let game = values["MC_VERSION"], let edition = values["OF_EDITION"], let release = values["OF_RELEASE"] else { throw RuriError.message(Messages.CoreOptiFineInstaller.releaseText2) }
    ...
    static func normalize(_ source: URL, to target: URL) throws {
        ...
        guard data.count >= 22, data.count <= 64 * 1024 * 1024 else { throw RuriError.message(Messages.CoreOptiFineInstaller.dataText1) }
        func number(_ offset: Int, _ count: Int) throws -> Int {
            guard offset >= 0, offset + count <= data.count else { throw RuriError.message(Messages.CoreOptiFineInstaller.numberText1) }
        ...
        guard let end = data.range(of: signature, options: .backwards, in: max(0, data.count - 65_557)..<data.count)?.lowerBound,
              ...
              try number(end + 4, 2) == 0, try number(end + 6, 2) == 0 else { throw RuriError.message(Messages.CoreOptiFineInstaller.endText1) }
        let count = try number(end + 10, 2), size = try number(end + 12, 4), start = try number(end + 16, 4)
        guard count < 30_000, start + size == end else { throw RuriError.message(Messages.CoreOptiFineInstaller.countText1) }
        var offset = start
        ...
        for _ in 0..<count {
            ...
            guard try number(offset, 4) == 0x02014b50 else { throw RuriError.message(Messages.CoreOptiFineInstaller.offsetText1) }
            let local = try number(offset + 42, 4)
            guard try number(local, 4) == 0x04034b50 else { throw RuriError.message(Messages.CoreOptiFineInstaller.localText1) }
            let length = try 46 + number(offset + 28, 2) + number(offset + 30, 2) + number(offset + 32, 2)
            guard local + 14 <= start, offset + length <= end else { throw RuriError.message(Messages.CoreOptiFineInstaller.lengthText1) }
        ...
        guard offset == end else { throw RuriError.message(Messages.CoreOptiFineInstaller.timeText1) }
    ...
    private static func read(_ name: String, in archive: Archive) throws -> Data {
        guard let entry = archive[name], entry.type == .file, entry.uncompressedSize <= 32 * 1024 * 1024 else { throw RuriError.message(Messages.CoreOptiFineInstaller.entryText1(String(describing: name))) }
        var data = Data(); let crc = try archive.extract(entry) { part in try Task.checkCancellation(); data.append(part) }
        guard crc == entry.checksum else { throw RuriError.message(Messages.CoreOptiFineInstaller.crcText1) }
        return data
    }
    private func publish(_ source: URL, to target: URL) throws {
        ...
        if FileManager.default.fileExists(atPath: target.path) {
            ...
            guard !protectExistingFiles, paths.repositoryImportID == nil else { throw RuriError.message(Messages.CoreOptiFineInstaller.publishText1(String(describing: target.lastPathComponent))) }
        }
        ...
        guard rename(staging.path, target.path) == 0 else { throw RuriError.message(Messages.CoreOptiFineInstaller.stagingText1) }
    }
}
