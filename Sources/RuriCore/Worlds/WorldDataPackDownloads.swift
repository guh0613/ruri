...
public actor WorldDataPackDownloads {
    ...
    public func prepare(_ version: ModrinthVersion, game: String) async throws -> WorldDataPackDownloadPlan {
        ...
        while !queue.isEmpty {
            ...
            guard seen.count <= 200 else { throw RuriError.message("数据包依赖数量超过限制。") }
            guard item.game_versions.contains(game), item.loaders.contains("datapack") else { throw RuriError.message("\(item.name) 没有适用于 Minecraft \(game) 的数据包版本。") }
            guard !resolved.contains(where: { $0.project_id == item.project_id && $0.id != item.id }) else { throw RuriError.message("必需依赖要求同一数据包的不同版本，请选择其他版本。") }
            ...
            for dependency in item.dependencies where dependency.dependency_type == "required" {
                if let id = dependency.version_id {
                    ...
                    guard let match = versions.first(where: { $0.version_type == "release" || $0.version_type == nil }) ?? versions.first else { throw RuriError.message("找不到兼容的数据包依赖：\(project)。") }
                    queue.append(match)
                } else { throw RuriError.message("数据包的必需依赖缺少项目或版本标识。") }
            }
        }
        for item in resolved {
            for dependency in item.dependencies where dependency.dependency_type == "incompatible" {
                if resolved.contains(where: { other in dependency.version_id.map { $0 == other.id } ?? (dependency.project_id == other.project_id) }) {
                    throw RuriError.message("\(item.name) 与必需依赖不兼容，无法一起安装。")
                }
            }
        }
        let files = try resolved.map { item in
            ...
            guard let file = archives.first(where: \.primary) ?? archives.first,
                ...
                throw RuriError.message("\(item.name) 缺少可校验的 ZIP 数据包文件，或文件超过 512 MB。")
            }
            return file
        }
        guard Set(files.map { $0.filename.lowercased() }).count == files.count else { throw RuriError.message("多个数据包使用相同的文件名，无法一起安装。") }
    ...
    public func install(_ plan: WorldDataPackDownloadPlan, instance: GameInstance, folder: String, paths: LauncherPaths, downloader: DownloadManager,
        ...
        guard let current = state.instances.first(where: { $0.id == instance.id }), current.gameVersion == plan.gameVersion else { throw RuriError.message("实例的游戏版本已改变，请重新选择数据包。") }
        ...
        for (version, file) in zip(plan.versions, plan.files) {
            ...
            await progress(InstallProgress("下载 \(version.name)", completed: sources.count, total: plan.files.count))
        ...
        try Task.checkCancellation()
        await progress(InstallProgress("正在导入数据包"))
