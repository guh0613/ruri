...
public actor WorldDataPackDownloads {
    ...
    public func prepare(_ version: ModrinthVersion, game: String) async throws -> WorldDataPackDownloadPlan {
        ...
        while !queue.isEmpty {
            ...
            guard seen.count <= 200 else { throw RuriError.message(Messages.CoreWorldDataPackDownloads.itemText1) }
            guard item.game_versions.contains(game), item.loaders.contains("datapack") else { throw RuriError.message(Messages.CoreWorldDataPackDownloads.itemText2(String(describing: item.name), String(describing: game))) }
            guard !resolved.contains(where: { $0.project_id == item.project_id && $0.id != item.id }) else { throw RuriError.message(Messages.CoreWorldDataPackDownloads.itemText3) }
            ...
            for dependency in item.dependencies where dependency.dependency_type == "required" {
                if let id = dependency.version_id {
                    ...
                    guard let match = versions.first(where: { $0.version_type == "release" || $0.version_type == nil }) ?? versions.first else { throw RuriError.message(Messages.CoreWorldDataPackDownloads.matchText1(String(describing: project))) }
                    queue.append(match)
                } else { throw RuriError.message(Messages.CoreWorldDataPackDownloads.matchText2) }
            }
        }
        for item in resolved {
            for dependency in item.dependencies where dependency.dependency_type == "incompatible" {
                if resolved.contains(where: { other in dependency.version_id.map { $0 == other.id } ?? (dependency.project_id == other.project_id) }) {
                    throw RuriError.message(Messages.CoreWorldDataPackDownloads.matchText3(String(describing: item.name)))
                }
            }
        }
        let files = try resolved.map { item in
            ...
            guard let file = archives.first(where: \.primary) ?? archives.first,
                ...
                throw RuriError.message(Messages.CoreWorldDataPackDownloads.fileText1(String(describing: item.name)))
            }
            return file
        }
        guard Set(files.map { $0.filename.lowercased() }).count == files.count else { throw RuriError.message(Messages.CoreWorldDataPackDownloads.fileText2) }
    ...
    public func install(_ plan: WorldDataPackDownloadPlan, instance: GameInstance, folder: String, paths: LauncherPaths, downloader: DownloadManager,
        ...
        guard let current = state.instances.first(where: { $0.id == instance.id }), current.gameVersion == plan.gameVersion else { throw RuriError.message(Messages.CoreWorldDataPackDownloads.currentText1) }
        ...
        for (version, file) in zip(plan.versions, plan.files) {
            ...
            await progress(InstallProgress(Messages.CoreWorldDataPackDownloads.destinationText1(String(describing: version.name)), completed: sources.count, total: plan.files.count))
        ...
        try Task.checkCancellation()
        await progress(InstallProgress(Messages.CoreWorldDataPackDownloads.destinationText2))
