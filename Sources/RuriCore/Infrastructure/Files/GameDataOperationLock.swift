...
final class GameDataOperationLock {
    ...
    func acquire(directory: URL, name: String) throws {
        ...
        guard fd >= 0 else { throw RuriError.message(Messages.CoreGameDataOperationLock.lockFailed) }
        ...
        guard fstat(fd, &info) == 0, info.st_mode & S_IFMT == S_IFREG, fcntl(fd, F_OFD_SETLK, &lock) == 0 else {
            close(fd); throw RuriError.message(Messages.CoreGameDataOperationLock.operationInProgress)
        }
