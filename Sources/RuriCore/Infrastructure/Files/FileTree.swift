...
public enum FileTree {
    ...
    static func entries(in root: URL, excluding: Set<String> = [], ignoringTransientFiles: Bool = true) throws -> [Entry] {
        guard try root.resourceValues(forKeys: [.isDirectoryKey, .isSymbolicLinkKey]).isSymbolicLink != true else { throw RuriError.message(Messages.CoreFileTree.symlinkDirectoryRequired) }
        ...
        while let (directory, prefix) = pending.popLast() {
            ...
            for url in try children(in: directory) {
                ...
                guard info.isSymbolicLink != true else { throw RuriError.message(Messages.CoreFileTree.symlinkInDirectory(String(describing: relative))) }
                guard info.isDirectory == true || info.isRegularFile == true else { throw RuriError.message(Messages.CoreFileTree.unsupportedFileType(String(describing: relative))) }
                ...
                guard entries.count <= 150_000 else { throw RuriError.message(Messages.CoreFileTree.tooManyFiles) }
    ...
    public static func copy(from source: URL, to destination: URL, excluding: Set<String> = [], progress: @Sendable (Int, Int) -> Void = { _, _ in }) throws {
        ...
        guard destinationPath != sourcePath, !destinationPath.hasPrefix(sourcePath + "/") else { throw RuriError.message(Messages.CoreFileTree.destinationInsideSource) }
        ...
        guard try Self.entries(in: source, excluding: excluding) == entries else { throw RuriError.message(Messages.CoreFileTree.sourceChangedDuringCopy) }
    }
    ...
    static func overlay(from source: URL, to destination: URL, excluding: Set<String> = []) throws {
        ...
        guard try Self.entries(in: source, excluding: excluding) == entries else { throw RuriError.message(Messages.CoreFileTree.overwrittenFileChanged) }
    }
}
...
extension SafeArchive {
    public static func create(from source: URL, to destination: URL, prefix: String = "", additionalFiles: [String: Data] = [:], excluding: Set<String> = [], progress: @Sendable (Int, Int) -> Void = { _, _ in }) throws {
        ...
        guard !destination.resolvingSymlinksInPath().standardizedFileURL.path.hasPrefix(root + "/") else { throw RuriError.message(Messages.CoreFileTree.archiveInsideSource) }
        ...
        func check(_ path: String) throws {
            guard !path.isEmpty, !path.hasPrefix("/"), !path.contains("\\"), !path.contains("\0"), !path.split(separator: "/", omittingEmptySubsequences: false).contains(where: { $0 == ".." || $0 == "." || $0.isEmpty }) else { throw RuriError.message(Messages.CoreFileTree.invalidArchivePath(path)) }
        }
        ...
        for name in entries.map({ prefix + $0.path }) + Array(additionalFiles.keys) {
            try check(name)
            guard names.insert(name).inserted else { throw RuriError.message(Messages.CoreFileTree.duplicateArchiveEntry(name)) }
        }
            ...
            for (index, entry) in entries.enumerated() {
                ...
                if entry.directory { try archive.addEntry(with: path + "/", type: .directory, uncompressedSize: Int64(0)) { _, _ in Data() } }
                    ...
                    try archive.addEntry(with: path, type: .file, uncompressedSize: entry.size, modificationDate: entry.modified, compressionMethod: .deflate, bufferSize: 128 * 1024) { position, count in
                        ...
                        guard let data = try handle.read(upToCount: count), data.count == count else { throw RuriError.message(Messages.CoreFileTree.sourceChangedDuringBackup(entry.path)) }
                        return data
                    }
                    let current = try entry.url.resourceValues(forKeys: [.fileSizeKey, .contentModificationDateKey])
                    guard current.fileSize == Int(entry.size), current.contentModificationDate == entry.modified else { throw RuriError.message(Messages.CoreFileTree.sourceChangedDuringBackup(entry.path)) }
                }
                progress(index + 1, entries.count)
            }
        ...
        guard current.count == entries.count, zip(current, entries).allSatisfy({ $0.path == $1.path && $0.size == $1.size && $0.modified == $1.modified }) else { throw RuriError.message(Messages.CoreFileTree.sourceChangedDuringBackupRetry) }
        try verify(staging, maxBytes: 128 * 1024 * 1024 * 1024)
        try Task.checkCancellation()
        guard rename(staging.path, destination.path) == 0 else { throw RuriError.message(Messages.CoreFileTree.archiveSaveFailed(destination.lastPathComponent)) }
    }
}
