...
struct FileTreeManifest: Codable, Equatable, Sendable {
    ...
    static func capture(in root: URL, excluding: Set<String> = [], ignoringTransientFiles: Bool = false) throws -> Self {
        ...
        guard try FileTree.entries(in: root, excluding: excluding, ignoringTransientFiles: ignoringTransientFiles) == before,
            ...
            throw RuriError.message(Messages.CoreFileTreeManifest.manifestChangedDuringVerification)
        }
        return result
    }
    ...
    static func capture(_ files: [FileTree.Entry], requiringDirectories: Set<String> = [], rootAttributes: [FileExtendedAttributes.Receipt]? = nil) throws -> Self {
        guard files.count <= 150_000 else { throw RuriError.message(Messages.CoreFileTreeManifest.tooManyFilesToVerify) }
        var result: [String: Entry] = [:], bytes: Int64 = 0
        for file in files {
            try Task.checkCancellation(); try validatePath(file.path)
            guard result[file.path] == nil else { throw RuriError.message(Messages.CoreFileTreeManifest.duplicateVerificationPath(file.path)) }
            if !file.directory {
                guard file.size >= 0, file.size <= maximumFileBytes - bytes else { throw RuriError.message(Messages.CoreFileTreeManifest.verificationFileTooLarge) }
                bytes += file.size
            }
            ...
            guard try FileExtendedAttributes.capture(file.url) == attributes else { throw RuriError.message(Messages.CoreFileTreeManifest.changedFileAttributes) }
        }
        ...
        for path in required {
            ...
            if let entry = result[path] {
                guard entry.directory else { throw RuriError.message(Messages.CoreFileTreeManifest.conflictingFilePath(path)) }
    ...
    func requireMatch(in root: URL, excluding: Set<String> = [], ignoringTransientFiles: Bool = false) throws {
        ...
        guard try Self.capture(in: root, excluding: excluding, ignoringTransientFiles: ignoringTransientFiles).matchingFormat(of: self) == self else {
            throw RuriError.message(Messages.CoreFileTreeManifest.fileContentMismatch)
        }
    }
    ...
    func requireRemainingMatch(in root: URL) throws {
        ...
        guard remaining.rootAttributes == rootAttributes, remaining.entries.allSatisfy({ expected[$0.path] == $0 }) else {
            throw RuriError.message(Messages.CoreFileTreeManifest.newOrChangedCleanupFiles)
        }
    }
    ...
    func save(to file: URL) throws -> String {
        ...
        guard data.count <= Self.maximumRecordBytes else { throw RuriError.message(Messages.CoreFileTreeManifest.verificationRecordTooLarge) }
        try data.write(to: file, options: .atomic)
        return Self.digest(data)
    }
    static func load(from file: URL, expectedDigest: String) throws -> Self {
        guard validDigest(expectedDigest) else { throw RuriError.message(Messages.CoreFileTreeManifest.invalidVerificationSummary) }
        let data = try RunDirectoryCopyGuard.read(file, limit: maximumRecordBytes)
        guard digest(data) == expectedDigest else { throw RuriError.message(Messages.CoreFileTreeManifest.changedVerificationRecord) }
        let manifest = try JSONDecoder().decode(Self.self, from: data)
        try manifest.validate(); return manifest
    }

    func validate() throws {
        guard (1...2).contains(version), entries.count <= 150_000 else { throw RuriError.message(Messages.CoreFileTreeManifest.invalidVerificationRecordVersion) }
        if let rootAttributes { guard version >= 2 else { throw RuriError.message(Messages.CoreFileTreeManifest.invalidVerificationRecordFileVersion) }; try FileExtendedAttributes.validate(rootAttributes) }
        var seen: [String: Bool] = [:], bytes: Int64 = 0
        for entry in entries {
            try Self.validatePath(entry.path)
            if version >= 2 {
                guard let attributes = entry.attributes else { throw RuriError.message(Messages.CoreFileTreeManifest.missingFileAttributes) }
                try FileExtendedAttributes.validate(attributes)
            } else if entry.attributes != nil { throw RuriError.message(Messages.CoreFileTreeManifest.invalidVerificationRecordFileVersion) }
            guard seen[entry.path] == nil, entry.size >= 0, entry.size <= Self.maximumFileBytes - bytes else { throw RuriError.message(Messages.CoreFileTreeManifest.duplicatePathsOrInvalidSizes) }
            if entry.directory {
                guard entry.size == 0, entry.sha256 == nil else { throw RuriError.message(Messages.CoreFileTreeManifest.invalidDirectoryRecord) }
            } else {
                guard entry.sha256.map(Self.validDigest) == true else { throw RuriError.message(Messages.CoreFileTreeManifest.invalidVerificationDigest) }
                bytes += entry.size
            }
            seen[entry.path] = entry.directory
        }
        for entry in entries {
            guard Self.parents(entry.path).allSatisfy({ seen[$0] == true }) else { throw RuriError.message(Messages.CoreFileTreeManifest.missingParentOrConflictingPath) }
        }
        guard entries.map(\.path) == entries.map(\.path).sorted() else { throw RuriError.message(Messages.CoreFileTreeManifest.invalidRecordOrder) }
    }
    ...
    private static func validatePath(_ path: String) throws {
        guard !path.isEmpty, path.utf8.count <= 4096, !path.contains("\\"), !path.contains("\0"),
              !path.split(separator: "/", omittingEmptySubsequences: false).contains(where: { $0.isEmpty || $0 == "." || $0 == ".." }) else { throw RuriError.message(Messages.CoreFileTreeManifest.invalidVerificationPath) }
    }
    ...
    private static func digest(_ file: URL, expectedSize: Int64) throws -> String {
        ...
        guard fd >= 0 else { throw RuriError.message(Messages.CoreFileTreeManifest.fileVerificationFailed(file.lastPathComponent)) }
        let handle = FileHandle(fileDescriptor: fd, closeOnDealloc: true); defer { try? handle.close() }
        var before = stat()
        guard fstat(fd, &before) == 0, before.st_mode & S_IFMT == S_IFREG, before.st_size == expectedSize else { throw RuriError.message(Messages.CoreFileTreeManifest.changedFileTypeOrSize) }
        var hash = SHA256(), remaining = expectedSize
        while remaining > 0 {
            try Task.checkCancellation()
            let data = try handle.read(upToCount: Int(min(1_048_576, remaining))) ?? Data()
            guard !data.isEmpty else { throw RuriError.message(Messages.CoreFileTreeManifest.changedFileLength) }
        ...
        guard fstat(fd, &after) == 0, lstat(file.path, &location) == 0,
              unchanged(before, after), unchanged(before, location) else { throw RuriError.message(Messages.CoreFileTreeManifest.fileModifiedOrReplaced) }
