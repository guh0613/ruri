...
extension FileTree {
    ...
    static func children(in directory: URL) throws -> [URL] {
        ...
        guard fd >= 0 else { throw RuriError.message(Messages.CoreFileTreeDirectory.directoryReadFailed(directory.lastPathComponent)) }
        guard let stream = fdopendir(fd) else { close(fd); throw RuriError.message(Messages.CoreFileTreeDirectory.directoryEnumerationFailed(directory.lastPathComponent)) }
        defer { closedir(stream) }
        var names: [String] = []
        while true {
            try Task.checkCancellation(); errno = 0
            guard let entry = readdir(stream) else {
                guard errno == 0 else { throw RuriError.message(Messages.CoreFileTreeDirectory.directoryEntryReadFailed) }; break
            }
            let length = Int(entry.pointee.d_namlen)
            let name = withUnsafePointer(to: &entry.pointee.d_name) {
                $0.withMemoryRebound(to: CChar.self, capacity: length + 1) { String(validatingCString: $0) }
            }
            guard let name else { throw RuriError.message(Messages.CoreFileTreeDirectory.unrepresentableFilename) }
            if name != "." && name != ".." { names.append(name) }
            guard names.count <= 150_000 else { throw RuriError.message(Messages.CoreFileTreeDirectory.tooManyDirectoryFiles) }
        }
        let all = Set(names)
