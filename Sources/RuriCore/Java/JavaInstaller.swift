...
public actor JavaInstaller {
    ...
    public func install(_ runtime: RemoteJava, downloader: DownloadManager, repairing: Bool = false, progress: @Sendable @escaping (InstallProgress) async -> Void) async throws -> JavaRuntime {
        ...
        if !repairing, FileManager.default.fileExists(atPath: destination.path) {
            ...
            throw RuriError.message(Messages.CoreJavaInstaller.javaCorrupt(destination.lastPathComponent))
        }
        ...
        guard repairing || !replacing else { throw RuriError.message(Messages.CoreJavaInstaller.javaAlreadyInstalled) }
        let manifestURL = try LauncherPaths.safePath("java-\(runtime.id).json", within: paths.cache)
        await progress(InstallProgress(Messages.CoreJavaInstaller.readJavaManifest(String(describing: runtime.major))))
        ...
        for (path, file) in manifest.files.sorted(by: { $0.key < $1.key }) {
            ...
            case "file":
                guard let artifact = file.downloads?["raw"] else { throw RuriError.message(Messages.CoreJavaInstaller.manifestDownloadInfoMissing) }
                downloads.append(DownloadItem(artifact, to: target))
                if file.executable == true { executables.append(target) }
            case "link":
                guard let link = file.target, !link.hasPrefix("/"), !link.contains("\\") else { throw RuriError.message(Messages.CoreJavaInstaller.insecureManifestLink) }
                let resolved = target.deletingLastPathComponent().appendingPathComponent(link).standardizedFileURL
                guard resolved.path.hasPrefix(staging.standardizedFileURL.path + "/") else { throw RuriError.message(Messages.CoreJavaInstaller.linkEscapesRuntime) }
                links.append((target, link))
            default: throw RuriError.message(Messages.CoreJavaInstaller.unsupportedJavaFileType(String(describing: file.type)))
            }
        }
        try await downloader.download(downloads) { done, total in await progress(InstallProgress(Messages.CoreJavaInstaller.downloadJava(String(describing: runtime.major)), completed: done, total: total)) }
        ...
        for (target, link) in links {
            if let existing = try? FileManager.default.destinationOfSymbolicLink(atPath: target.path) {
                guard existing == link else { throw RuriError.message(Messages.CoreJavaInstaller.inconsistentJavaLinks) }
            } else {
                try FileManager.default.createDirectory(at: target.deletingLastPathComponent(), withIntermediateDirectories: true)
                try FileManager.default.createSymbolicLink(atPath: target.path, withDestinationPath: link)
            }
            guard target.resolvingSymlinksInPath().path.hasPrefix(staging.resolvingSymlinksInPath().path + "/") else { throw RuriError.message(Messages.CoreJavaInstaller.linkEscapesRuntime) }
        }
        try Task.checkCancellation()
        let checked = try JavaDiscovery.inspect(staging.appendingPathComponent("jre.bundle/Contents/Home/bin/java").path)
        guard checked.major == runtime.major, checked.architecture == runtime.architecture else { throw RuriError.message(Messages.CoreJavaInstaller.javaVersionMismatch) }
        try JSONEncoder().encode(runtime).write(to: staging.appendingPathComponent(".ruri-runtime.json"), options: .atomic)
        if replacing {
            try JavaRuntimeLease.requireNoRunningProcess(in: destination)
            guard renamex_np(staging.path, destination.path, UInt32(RENAME_SWAP)) == 0 else { throw RuriError.message(Messages.CoreJavaInstaller.javaDirectoryReplaceFailed) }
            do {
                let result = try JavaDiscovery.inspect(binary.path)
                try? FileManager.default.removeItem(at: staging)
                return result
            } catch {
                guard renamex_np(staging.path, destination.path, UInt32(RENAME_SWAP)) == 0 else { throw RuriError.message(Messages.CoreJavaInstaller.repairedJavaUnrunnable(staging.path)) }
                throw error
            }
        }
