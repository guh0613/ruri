...
extension CLI {
    static func manageModpackUpdate(_ args: [String], paths: LauncherPaths) async throws {
        guard args.count >= 2, let id = UUID(uuidString: args[1]), let instance = try StateStore.load(paths).instances.first(where: { $0.id == id }) else {
            throw RuriError.message("用法：update-pack <instance-uuid> <archive> [--apply] [--replace-local]；rollback-pack/recover-pack-update <instance-uuid> [--apply]")
        }
        let service = ModpackUpdater(paths: paths)
        if args[0] == "recover-pack-update" || args[0] == "rollback-pack" {
            guard args.count == 2 || (args.count == 3 && args[2] == "--apply") else { throw RuriError.message("仅支持 --apply 参数。") }
            if args[0] == "recover-pack-update" {
                print(ModpackUpdateStore.hasPending(paths: paths, instanceID: id) ? "更新需要恢复" : "没有待恢复的更新")
                if args.count == 3 { _ = try ModpackUpdateStore.recover(instanceID: id, paths: paths) }
            } else {
                print(ModpackUpdateStore.hasBackup(paths: paths, instanceID: id) ? "可回退上次更新；更新后的本地文件修改会保留" : "没有更新备份")
                if args.count == 3 {
                    let result = try await service.rollback(instance)
                    print("已回退，保留 \(result.preservedFiles) 项后续修改")
                }
            }
            return
        }
        guard args.count >= 3, Set(args.dropFirst(3)).isSubset(of: ["--apply", "--replace-local"]) else { throw RuriError.message("用法：update-pack <instance-uuid> <archive> [--apply] [--replace-local]") }
        ...
        do {
            ...
            print("Minecraft \(instance.gameVersion) → \(preview.incoming.settings.gameVersion)，\(preview.incoming.settings.loader.title)")
