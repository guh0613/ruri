...
extension Messages {
    public enum CLILaunchSettingsCommands {
        /// 用法：launch-settings <defaults|实例UUID> [set <memory|initialMemory|metaspace|java|jvmArguments|gameArguments|window|fullscreen|presentation|debugLogging|macOS|nativeFullscreen|instanceAppearance|environment> <值> | inherit <项目|all>]。memory 为 auto 或 MB；initialMemory 为 default 或 MB；metaspace 为 unlimited 或 MB。Java 为 auto、主版本号或完整路径，窗口为 1600x900；fullscreen 为 true/false；presentation 为 keep、hide 或 logs；debugLogging 为 true/false，默认关闭。environment 为每行一项 NAME=value，只写 NAME 移除继承值；environment-file 可从 UTF-8 文件读取，空值清除本实例配置。commands true/false 控制运行自定义命令；preLaunchCommand、postExitCommand、commandWrapper 设置命令文本，commandTimeout 为秒数。macOS、nativeFullscreen、instanceAppearance 使用 true/false 控制 macOS 游戏集成、原生全屏和实例外观。默认只查看生效值与继承来源。
        ///
