...
struct StateStoreTests {
    ...
    @Test func staleWriterWithoutBaselineCannotEraseNewerAccounts() throws {
        ...
        var stale = baseline; stale.settings.defaultMemorySettings = .init(maximumMB: 8192)
        #expect(throws: (any Error).self) { try StateStore.save(stale, to: paths) }
        #expect(try StateStore.load(paths).accounts == other.accounts)
        #expect(try StateStore.load(paths).settings.defaultMemorySettings == nil)
    }
    @Test func concurrentInstallAndPreferencesPreserveBothAndAllowFollowingSave() throws {
        ...
        var local = baseline; local.settings.defaultMemorySettings = .init(maximumMB: 8192); local.instances[0].favorite = true
        ...
        #expect(merged.instances[0].favorite && merged.settings.defaultMemorySettings?.maximumMB == 8192)
    ...
    @Test func failedMutationAndInvalidReferencesLeaveStateBytesUntouched() throws {
        ...
        #expect(throws: (any Error).self) {
            try StateStore.update(paths) { state in
                state.settings.defaultMemorySettings = .init(maximumMB: 8192)
    ...
    @Test func aSecondWriterCannotEnterAnActiveTransaction() throws {
        ...
        try StateStore.update(paths) { state in
            ...
            state.settings.defaultMemorySettings = .init(maximumMB: 6144)
        }
        #expect(try StateStore.load(paths).settings.defaultMemorySettings?.maximumMB == 6144)
